...
impl AgentExecutor {
    pub fn new(base_url: &str) -> Self {
        ...
        if let Ok(_) = rag.load_from_json("vector_db/knowledge_base.json") {
            println!("[RUST CORE] Successfully loaded knowledge_base.json");
        } else {
            println!("[RUST CORE] Warning: knowledge_base.json not found or failed to load.");
        }
        ...
        let native_llm = if std::path::Path::new("models/SmolLM2-1.7B-Instruct-Q4_K_M.gguf").exists() {
            println!("[RUST CORE] Found local GGUF model. Initializing native inference...");
            NativeInference::new(
    ...
    pub async fn execute(
        ...
        while current_step != WorkflowStep::End {
            match current_step {
                ...
                WorkflowStep::Vision => {
                    println!("[RUST CORE] Node: Vision Analysis");
                    current_step = WorkflowStep::Retriever;
                }
                WorkflowStep::Retriever => {
                    println!("[RUST CORE] Node: Retriever (Rust-Native Search)");
                ...
                WorkflowStep::Drafter => {
                    ...
                    if let Some(local_llm) = &self.native_llm {
                        println!("[RUST CORE] Using Native Local Inference");
                        ...
                        println!("[RUST CORE] Using Remote OAI Inference");
                ...
                WorkflowStep::Review => {
                    println!("[RUST CORE] Node: Collaborative Review");
                ...
                WorkflowStep::ConflictDetector => {
                    println!("[RUST CORE] Node: Consensus Engine");
                ...
                WorkflowStep::Synthesizer => {
                    println!("[RUST CORE] Node: Final Synthesis");
                ...
                WorkflowStep::SimpleResponse => {
                    println!("[RUST CORE] Node: Simple Responder");
